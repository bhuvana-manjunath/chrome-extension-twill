...
function handleMessage(event) {
    ...
    const messageActions = {
        ...
        openFloatingModal: () => openFloatingModal(reqMessage.url, reqMessage.height, reqMessage.width),
...
function getStorageAction(context, actionType) {
    return localStorageFunctions[context][actionType];
}

// Define the available storage functions
const localStorageFunctions = {
    ...
    activeTab: {
        write: writeToLocalStorageInTab,
        read: readFromLocalStorageInTab,
        delete: deleteFromLocalStorageInTab,
        clear: clearLocalStorageInTab
    },
    chromeStorageLocal: {
...
// Function: open floating modal
function openFloatingModal(url, height, width) {
    sendMessageToBackground({ type: "openFloatingModal", url: url, height: height, width: width });
}
...
function setElementText(selector, text) {
    executeScriptInActiveTab(setElementTextInDOM, [selector, text]);
}

// Function: set element text (injection function)
function setElementTextInDOM(selector, text) {
    var element = document.querySelector(selector);
    if (!element) return 
    element.innerText = text;
}

// Function to set element HTML
function setElementHTML(selector, html) {
    executeScriptInActiveTab(setElementHTMLInDOM, [selector, html]);
}

// Function: set element HTML (injection function)
function setElementHTMLInDOM(selector, html) {
    var element = document.querySelector(selector);
    if (!element) return 
...
function clickOnElement(selector) {
    executeScriptInActiveTab(clickOnElementInDOM, [selector]);
}
// Function: click on element (injection function)
function clickOnElementInDOM(selector) {
    var element = document.querySelector(selector);
    if (!element) return 
    element.click();
}
...
function captureScreenshot() {
    chrome.tabs.captureVisibleTab(null, { format: "png" }, function (dataUrl) {
        var msg = { returnType: "screenshot", dataUrl: dataUrl };
        postMessageToPlugin(msg);
    });
}
